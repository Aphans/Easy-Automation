<template>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg">
      ...
      <form class="grid grid-cols-2 gap-4" @submit.prevent="addDevice">
        <div class="flex flex-col">
          ...
          <input id="name" v-model="form.name" class="p-2 rounded-lg border-gray-300" type="text" required>
        </div>
        <div class="flex flex-col">
          <label for="type" class="mb-1 font-medium">Type:</label>
          <select id="type" v-model="form.type" class="p-2 rounded-lg border-gray-300" required>
        ...
        <div class="flex flex-col" v-if="form.type === 'Sensor'">
          ...
          <input id="value" v-model="form.value" class="p-2 rounded-lg border-gray-300" type="number" required>
        </div>
  ...
  :class="form.value === 'OFF' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'"
...
>
  {{ form.value }}
        ...
        <div class="col-span-2">
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg">Add Device</button>
          <button type="button" class="ml-4 text-gray-600 font-medium py-2 px-4 rounded-lg" @click="cancelForm">Cancel</button>
        </div>
      </form>
    </div>
  </div>
